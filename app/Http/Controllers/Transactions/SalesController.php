...
class SalesController extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('transactions.sales_order.index', compact('title'));
    }
        ...
        return Datatables::of($data)->addIndexColumn()
            ->addColumn('action', function ($row) {
                ...
                if (Gate::allows('crudAccess', 'TX2', $row)) {
                    if ($row->status == 1) {
                        $btn = '<a href="/transaction/sales?uid=' . $row->uid . '" class="btn btn-dim btn-outline-secondary btn-sm"><em class="icon ni ni-edit"></em><span>Edit</span></a>&nbsp;
